...
public class IamRepositoryImpl implements RefreshTokenRepository {
    ...
    @Autowired
    public IamRepositoryImpl(InfrastructureHelper infrastructureHelper,
    ...
    public RefreshTokenEntity createRefreshToken(RefreshTokenEntity refreshToken) {
        try {
            Timestamp expiry_date = infrastructureHelper.getExpiryDate(10 /* minutes */);
            String token = UUID.randomUUID().toString();

            UserEntity user = userRepository.getUser(refreshToken.getUser().getUserGuid());
            ...
            PreparedStatementCreator preparedStatementCreator = connection -> {
                ...
                ps.setString(0, token);
                ps.setTimestamp(1, expiry_date);
            ...
            long autogeneratedId = Objects.requireNonNull(keyHolder.getKey()).longValue();
            refreshToken.setId(autogeneratedId);
            refreshToken.setToken(token);
            refreshToken.setExpiryDate(expiry_date);
    ...
    private RefreshTokenEntity toRefreshTokenEntity(ResultSet rs) throws SQLException {
        ...
        if (rs.next()) {
            ...
            refreshTokenEntity.setExpiryDate(rs.getTimestamp(RefreshTokenQueries.EXPIRY_DATE_COLUMN.toLowerCase()));
